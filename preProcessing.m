function preProcessing(rgbImage)
...
% Generate a noisy image(salt and pepper noise).  
...
% Extract the individual red, green, and blue color channels.
redChannel = rgbFixed(:, :, 1);
greenChannel = rgbFixed(:, :, 2);
blueChannel = rgbFixed(:, :, 3);

% Display the individual red, green, and blue color channels.
subplot(3, 4, 10);
imshow(redChannel);
title('Noise Free Red Channel', 'FontSize', fontSize);
subplot(3, 4, 11);
imshow(greenChannel);
title('Noise Free Green Channel', 'FontSize', fontSize);
subplot(3, 4, 12);
imshow(blueChannel);
title('Noise Free Blue Channel', 'FontSize', fontSize);
